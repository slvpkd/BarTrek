...
function showData(place,i,distance) {
	...
	var listingInfo = {
        addButton: "<button class='btn btn-block active btn-lg btn-primary' onClick='addToRoute(" + i + ");'><span class='fa fa-map-signs'></span> Add " + place.name + "</button>",
...
function getPhoto(imgUrl,w,h) {
	return imgUrl.getUrl({'maxWidth': w, 'maxHeight': h});
}  
}
...
function getWebsite(place) {
	...
	if (!website) {
		...
		website = `<a href="` + website + `" target="_blank">` + website + ` <i class="fa fa-external-link-square-alt"></i></a>`;
    }
    return website;
}
function getReviews(place) {
	...
	if (reviews.length > 0) {
		for (i = 0; i < reviews.length; i++) {
			...
			reviewText += `
                <li class="media">
                    <img class="mr-3" src="` + place.reviews[i].profile_photo_url + `" width="32px" height="32px" alt="Generic placeholder image">
...
listingClass = "secondary";
}
//console.log(listingClass);

listingHTML = `
<div class="card bg-`+listingClass+`">
	<div class="card-body">
		<div class="row">

			<div class="col-md">
				<div class="card card200">
				<div class="card-header bg-primary"><h4><strong>`+x.placeObj.name+` <span class="badge">`+x.placeRating+`</span></strong></h4></div>
					<div class="card-body">
			...
			<div class="col-md">
				<div class="card card200">
				<div class="card-header bg-primary"><h4>Opening Hours</h4></div>
					<div class="card-body">
					`+x.placeHours+`
					</div>
				</div>
		...
		<div class="col-md">
			<div class="card card200">
			<div class="card-header bg-primary"><h4>Reviews</h4></div>
				<div class="card-body">
				`+x.placeReviews+`
				</div>
			</div>
...
function getIWContent(place, i) {
	...
	var content = "<h5>"+place.name+"</h5><button class='btn btn-block btn-sm active btn-outline-primary' onClick='addToRoute(" + i + ");'>Add</button>";
	//place.name;
